      ...
      <div class="min-w-0">
        ...
        <div class="mt-8 mb-6 flex flex-col gap-3">
          <div class="flex justify-center">
            <div
              class="flex flex-wrap gap-2 sm:gap-3 bg-white/80 border border-gray-200 rounded-full p-1.5 shadow-sm"
            >
              <button
                v-for="tab in tabs"
                :key="tab"
        ...
        <article
          v-if="featured"
          class="relative overflow-hidden rounded-2xl border border-gray-200 shadow-sm group mb-8"
        >
          <div class="h-64 sm:h-80 w-full">
            ...
            <template v-else>
              <div
                class="h-full w-full bg-gradient-to-br from-slate-200 to-slate-300"
              ></div>
            </template>
          </div>
          ...
          <div class="absolute left-4 right-4 bottom-4">
            <div class="flex items-center gap-2">
              ...
              <span class="text-white/80 text-xs">
                <template v-if="featured.type === 'event'">
                  ...
                  <template v-if="featured.time"
                    >• {{ formatDisplayTime(featured.time) }}</template
                  >
                </template>
                <template v-else>
                  {{ formatDisplayDate(featured.date) }}
            ...
            <p
              ...
              class="mt-2 text-white/90 line-clamp-2 hidden sm:block"
            >
          ...
          >
            <div class="relative">
              ...
              <div class="absolute left-3 top-3">
                <div
                  class="flex flex-col items-center justify-center w-14 h-14 rounded-xl bg-white/95 border border-white/60 shadow-sm"
                >
                  <div
                    class="text-[10px] font-semibold text-gray-500 leading-none"
                  >
                    {{ monthAbbrev(item.date).toUpperCase() }}
                  </div>
                  <div
                    class="text-lg font-extrabold text-gray-800 leading-none"
                  >
            ...
            <div class="p-5">
                ...
                <span class="text-xs text-gray-500">
                  <template v-if="item.type === 'event'">
                    ...
                    <template v-if="item.time"
                      >• {{ formatDisplayTime(item.time) }}</template
                    >
                  </template>
                  <template v-else>
                    {{ formatDisplayDate(item.date) }}
              ...
              <RouterLink
                :to="routeFor(item)"
                class="text-base font-bold text-[#002060] group-hover:underline underline-offset-4 line-clamp-2"
              >
                {{ item.title }}
              </RouterLink>
              ...
              <p
                ...
                class="mt-2 text-gray-700 line-clamp-3"
              >
                {{ item.description }}
              </p>
...
const activeTab = ref(
  tabs.includes(String(route.query.tab)) ? String(route.query.tab) : "All"
);
...
const newsItems = computed(() =>
  latestNewsSorted().map((n) => ({ ...n, type: "news" }))
);
const eventItems = computed(() =>
  upcomingEventsSorted().map((e) => ({ ...e, type: "event" }))
);

// combined (for All)
const toDate = (it) => new Date(`${it.date}T${it.time || "00:00"}:00`);
const allItems = computed(() =>
  [...newsItems.value, ...eventItems.value].sort(
...
/** Route builder:
 * - Events -> { name: 'events-detail' }
 * - News   -> { name: 'news-detail', params: { slug: item.id } }
 *   (uses the NEWS id as a stable slug so you don't have to edit data)
 */
function routeFor(item) {
  if (item?.type === "event") {
    return { name: "events-detail" };
  }
  return { name: "news-detail", params: { slug: item?.id || "news" } };
}
</script>
