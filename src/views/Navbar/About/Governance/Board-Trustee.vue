<template>
  ...
  <div
    class="grid grid-cols-1 md:grid-cols-[65%_35%] lg:grid-cols-[70%_30%] mx-auto 2xl:max-w-[1320px] xl:max-w-[1152px] lg:max-w-[1024px] sm:max-w-[600px] max-w-[300px] gap-6 md:gap-10"
  >
    <!-- Left -->
    <div class="flex flex-col mb-10 gap-6 md:gap-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 items-start">
        <div class="order-2 md:order-1 space-y-2">
          <span class="font-bold text-xl sm:text-2xl md:text-3xl">{{
            selectedBoard.name
          }}</span>
          <p class="text-base sm:text-lg md:text-xl text-gray-700 leading-snug">
            {{ selectedBoard.position }}
          </p>
        </div>
        <div
          class="order-1 md:order-2 overflow-hidden rounded-lg border border-gray-300 shadow-lg"
        >
          <img
      ...
      <div class="relative">
        <div
          class="flex items-center gap-3 mt-2 md:mt-4 py-3 px-10 md:px-12 rounded-lg relative overflow-hidden"
        >
          <!-- Prev -->
          <button
          ...
          <div class="w-full overflow-hidden">
            <div
              class="flex gap-3 md:gap-4 transition-transform duration-300 ease-out"
            >
              <div
                v-for="(board, index) in visibleBoards"
                :key="board.name"
                class="flex-shrink-0 w-24 sm:w-28 md:w-32 lg:w-36 flex flex-col items-center rounded-xl cursor-pointer p-2 transition-all border border-transparent"
                :class="[
                  'snap-start',
                  selectedIndex === startIndex + index
                    ? 'bg-gray-100 ring-2 ring-usea_secondary'
              ...
              >
                <img
                  :src="board.img"
                  :alt="board.name"
                  class="object-cover rounded-full transition-transform duration-300 hover:scale-105 w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28"
                />
                <span
                  class="font-semibold text-sm sm:text-base text-center mt-2 leading-tight line-clamp-2"
                >
                  {{ board.name }}
                </span>
                <p
                  class="text-xs sm:text-sm text-gray-600 text-center line-clamp-2"
                >
                  {{ board.position }}
                </p>
              </div>
          ...
          <!-- Next -->
          <button
            class="absolute right-2 top-1/2 -translate-y-1/2 bg-gray-700 text-white p-2 md:p-3 rounded-full hover:bg-red-700 z-10 disabled:opacity-40 disabled:cursor-not-allowed"
            @click="scrollRight"
...
const boards = [
  ...
  { img: YimChanVisal, name: "Mr. Yim Chanvisal", position: "Vice President" },
  {
    img: TealYuLong,
    name: "Mr. Teal Yulong",
    position: "Dean of Faculty Economic Business and Tourism",
  },
  {
    img: SaSokNgim,
    name: "Mrs. Sa Sokngim",
    position: "Dean of Faculty Sciences and Technology",
  },
  {
    img: leangTaingSoth,
    name: "Mr.Leang Taingsoth",
    position: "Dean of Faculty Social Science and Law",
  },
  {
    img: TatBunSeang,
    name: "Mr. Tat Bunseang",
    position: "Director of IT Center",
  },
];
