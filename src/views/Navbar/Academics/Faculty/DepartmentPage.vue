    ...
    <div>
      <section class="mb-12">
        ...
        <div class="flex flex-col gap-5">
          ...
          <p class="text-justify text-md md:text-md">
      ...
      <section class="mb-12">
        <div class="flex flex-col justify-center items-center mb-5">
          <span class="text-usea_secondary text-[2rem] font-bold">MAJORS</span>
        ...
        <div
          v-if="activeTab === `General Chinese Programs`"
          class="grid 2xl:grid-cols-3 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 gap-5 mb-5"
        >
          <div
            class="group relative max-w-xl text-center mx-auto cursor-pointer overflow-hidden rounded-md"
            @click="
              goToMajor(
                major.params.majorDepartment,
                major.params.majorDegree,
                major.params.majorName
              )
            "
            v-for="major in department.majors.general"
            :key="major.name"
          >
            <img
              :src="major.image"
            ...
            >
              <h2 class="text-white text-2xl p-3 font-bold">
                {{ major.name }}
              </h2>
            </div>
          </div>
        ...
        <div
          v-if="activeTab === `Associate's Degree`"
          class="grid 2xl:grid-cols-3 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 gap-5 mb-5"
        >
          <div
            class="group relative max-w-xl text-center mx-auto cursor-pointer overflow-hidden rounded-md"
            ...
            "
            v-for="major in department.majors.associate"
            :key="major.name"
          >
            <img
              :src="major.image"
        ...
        <div
          v-if="activeTab === `Bachelor's Degree`"
          class="grid 2xl:grid-cols-3 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 gap-5 mb-5"
        >
          <div
            class="group relative max-w-xl text-center mx-auto cursor-pointer overflow-hidden rounded-md"
            ...
            "
            v-for="major in department.majors.bachelor"
            :key="major.name"
          >
            <img
              :src="major.image"
      ...
      <section>
        ...
        <div
          class="grid 2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 gap-4 mb-5"
        >
          <div v-for="(activity, index) in department.activities" :key="index">
            <div class="overflow-hidden">
              <img
                :src="activity.image"
                alt="activity"
                class="w-full object-cover transform transition-transform duration-300 hover:scale-105"
              />
            </div>
          </div>
        </div>
...
const goToMajor = (majorDepartment, majorDegree, majorName) => {
  router.push({
    name: "major-name",
    params: { majorDepartment, majorDegree, majorName },
  });
};
</script>
