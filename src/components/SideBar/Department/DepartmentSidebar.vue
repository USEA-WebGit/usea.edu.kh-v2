<template>
  ...
  >
    ...
    <section class="space-y-3" v-if="department">
      ...
      >
        <span class="text-[#002060] text-sm md:text-sm font-bold"
          >Department +</span
        >
        <svg
      ...
      <nav v-show="open.contact" class="flex flex-col text-sm md:text-sm">
        <RouterLink
          :to="{
            name: routes.aboutDept,
            params: { [paramKeys.departmentName]: departmentKey },
          }"
          class="nav-link"
          :class="{
            active: isActive({
    ...
    <section class="space-y-3" v-for="cat in categories" :key="cat.key">
      ...
      >
        ...
        <svg
          class="h-5 w-5 transition-transform"
          :class="open.cats[cat.key] ? 'rotate-180' : ''"
          viewBox="0 0 20 20"
          fill="currentColor"
        >
          <path
      ...
      <nav v-show="open.cats[cat.key]" class="flex flex-col text-sm md:text-sm">
        <RouterLink
...
const categories = computed(() => {
  ...
  const out = [];
  if (m.general?.length)
    out.push({ key: "general", label: "General Chinese Programs" });
  if (m.associate?.length)
    out.push({ key: "associate", label: "Associate's Degree" });
  if (m.bachelor?.length)
    out.push({ key: "bachelor", label: "Bachelor's Degree" });
  return out;
});
