<!-- src/components/Events/Events.vue (homepage section) -->
<template>
...
/* Optional: limit the number of items shown on the homepage */
const props = defineProps({
  limit: { type: Number, default: 4 },
});

/* Raw items from centralized dataset, already sorted soonest-first */
...
const eventsList = computed(() =>
  upcoming.value.map((ev) => ({
    ...
    venue: ev.location || "",
    // pass the full item through router state + a shareable ?id= fallback
    to: { name: "events-detail", query: { id: ev.id }, state: { item: ev } },
  }))
);

/* Use the prebuilt link if present (kept flexible for future), else build one */
function routeFor(e) {
  return (
    e.to || { name: "events-detail", query: { id: e.id }, state: { item: e } }
  );
}
</script>
