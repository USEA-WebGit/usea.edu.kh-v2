<template>
  ...
  <div class="mx-auto 2xl:max-w-[1320px] xl:max-w-[1152px] lg:max-w-[1024px] sm:max-w-[600px] max-w-[95%]">
    <article v-if="article" class="py-8">
      <header class="mb-5">
        ...
        <div class="mt-2 text-sm text-gray-500 flex flex-wrap gap-4">
          <span><i class="fa-solid fa-calendar-days"></i> {{ article.displayDate }}</span>
          <span v-if="article.displayTime"><i class="fa-solid fa-clock"></i> {{ article.displayTime }}</span>
          <span v-if="article.venue"><i class="fa-solid fa-location-dot"></i> {{ article.venue }}</span>
        </div>
      </header>

      <figure v-if="article.img" class="mb-6 overflow-hidden rounded-xl border border-gray-200">
      ...
      <div class="prose max-w-none prose-p:leading-relaxed prose-headings:mt-6 prose-headings:mb-3">
        <p v-if="article.excerpt" class="text-gray-800 mb-4">{{ article.excerpt }}</p>
        <p class="text-gray-800">{{ article.body || fallbackBody }}</p>
      </div>

      <div v-if="article.gallery?.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
        <img
    ...
    <div v-else class="py-16 text-center text-gray-600">
      ...
      <router-link :to="{ name: 'news-events', query: { tab: 'News' } }" class="text-usea_secondary font-semibold hover:underline">
        Back to News
      </router-link>
    </div>
  </div>
...
import { NEWS, formatDisplayDate, formatDisplayTime } from "@/data/news-events.js";

const route = useRoute();
const slug = computed(() => String(route.params.slug ?? "")); // params are strings

// Prefer router state if provided; else find by id
const raw = computed(() => {
  const fromState = history.state && history.state.item;
  if (fromState && (!slug.value || String(fromState.id) === slug.value)) return fromState;
  return NEWS.find((n) => String(n.id) === slug.value) || null;
});
...
const fallbackBody = "Details coming soon. Please check back shortly for the full story and photos.";
</script>
